...
function fromProduct(p: any) {
  return {
    ...
    wash_temperature: p.wash_temperature,
    dry_cleaning: p.dry_cleaning,
...
export default function ProductFormModal({ product, onClose, onSaved }: any) {
  ...
  const setMaterial = (i: number, k: string) => (e: any) => {
    setForm((f: any) => {
      ...
      mats[i] = {
        ...mats[i],
        [k]: k === "is_recycled" ? e.target.checked : e.target.value,
      };
  ...
  const validate = () => {
    ...
    if (!form.country_of_production.trim())
      errs.country = t("productForm.errors.country");
    if (!form.production_date) errs.date = t("productForm.errors.date");
    if (form.materials.length > 0) {
      const total = form.materials.reduce(
        (s: number, m: any) => s + (parseFloat(m.percentage) || 0),
        0,
      );
      if (Math.abs(total - 100) > 0.01)
        errs.materials = t("productForm.errors.materials", {
          total: total.toFixed(1),
        });
      if (form.materials.some((m: any) => !m.name.trim()))
        errs.materials = errs.materials || t("productForm.errors.materialName");
    }
    return errs;
  };
  ...
  const totalPct = form.materials.reduce(
    (s: number, m: any) => s + (parseFloat(m.percentage) || 0),
    0,
  );
      ...
      <div className="card w-full max-w-2xl mb-6">
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-5">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="label">{t("productForm.name")}</label>
              <input
                className="input"
                value={form.name}
                onChange={set("name")}
            ...
            <div>
              <label className="label">{t("productForm.brand")}</label>
              <input
                className="input"
                value={form.brand}
                onChange={set("brand")}
            ...
            <div>
              <label className="label">{t("productForm.category")}</label>
              <select
            ...
            <div>
              <label className="label">{t("productForm.country")}</label>
              <input
                className="input"
                value={form.country_of_production}
                onChange={set("country_of_production")}
                placeholder={t("productForm.countryPlaceholder")}
              />
              {errors.country && (
            ...
            <div>
              <label className="label">{t("productForm.date")}</label>
              <input
                className="input"
                type="date"
            ...
            <div>
              <label className="label">{t("productForm.washTemp")}</label>
              <select
                className="input appearance-none cursor-pointer"
                value={form.wash_temperature}
                onChange={set("wash_temperature")}
              >
                {WASH_TEMPS.map((temp) => (
                  <option key={temp} value={temp}>
                    {temp === "none" ? t("productForm.noWash") : `${temp}°C`}
                  </option>
                ))}
              </select>
            </div>
          ...
          <div className="flex items-center gap-3">
            <input
              type="checkbox"
              id="dry"
              checked={form.dry_cleaning}
              onChange={setCheck("dry_cleaning")}
            ...
            <label
              ...
              style={{ color: "var(--text)" }}
            >
              {t("productForm.dryCleaning")}
            </label>
          </div>
          <div>
            <label className="label">{t("productForm.notes")}</label>
            <textarea
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              ...
              <span
                ...
                style={{
                  color: totalPct === 100 ? "var(--brand)" : "var(--warning)",
                }}
              >
            ...
            {errors.materials && (
              <p className="text-xs text-red-500 mb-2">{errors.materials}</p>
            )}
            <div className="space-y-2">
              {form.materials.map((m: any, i: number) => (
                <div key={i} className="flex gap-2 items-center">
                  <input
                    className="input"
                    placeholder={t("productForm.materialName")}
                    value={m.name}
                    onChange={setMaterial(i, "name")}
                  />
                  <input
                    ...
                    placeholder="%"
                    value={m.percentage}
                    onChange={setMaterial(i, "percentage")}
                  />
                  <label
                    className="flex items-center gap-1.5 text-xs whitespace-nowrap cursor-pointer shrink-0"
                    style={{ color: "var(--text-muted)" }}
                  >
                    <input
                      type="checkbox"
                      checked={m.is_recycled}
                      onChange={setMaterial(i, "is_recycled")}
